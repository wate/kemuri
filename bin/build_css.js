...
class buildCSS {
    ...
    if (indexFileEntries.length > 0) {
      indexEntryContents = indexFileEntries.map((entry) => {
        ...
        if (entry.comments && entry.comments.length > 0) {
          //コメントを追加出力
          indexEntryString += entry.comments.map((comment) => '// ' + this.indexCommentTag + ' ' + comment).join('\n');
          indexEntryString += '\n';
        }
  ...
  generateIndexFiles() {
    ...
    // メインファイルの生成
    if (this.mainName && (mainFileEntries['subDirectoryIndexes'].length > 0 || mainFileEntries['file'].length > 0)) {
    ...
    this.sassFiles.forEach((compileSassFilePath) => {
      ...
      try {
        if (!fs.existsSync(path.dirname(cssOutputPath))) {
          fs.mkdirSync(path.dirname(cssOutputPath), { recursive: true }, (err) => {
            if (err) throw err;
          });
        }
...
const sassLoadPaths = [srcDir, process.cwd() + path.sep + 'node_modules'];
